...
void OnDataRecv(const uint8_t * mac, const uint8_t *incomingData, int len) {
    ...
    // Re-send data struct to the receiver board
    esp_err_t result = esp_now_send(receiverMAC, (uint8_t *) &getData, sizeof(getData));
    
    if (result == ESP_OK) {
        Serial.println("Re-sent OK");
    } else {
        Serial.println("Re-sent FAIL");
    }
}
...
void loop() {
    ...
    if (millis() - time_stamp > 2000){
        ...
        sendData.id = nodeID;
        sendData.x = random(100);
        sendData.y = random(100);
